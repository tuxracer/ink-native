...
A library for rendering [Ink](https://github.com/vadimdemedes/ink) TUI applications in native windows. Zero system dependencies — uses an embedded bitmap font and a bundled native framebuffer library.

See [README.md](README.md) for API documentation and usage examples. See [docs/TRD.md](docs/TRD.md) for technical details including architecture, ANSI sequence support, and keyboard input mapping.

**Repository URL**: https://github.com/tuxracer/ink-native
...
- `native/` - Fenster native C bridge (compiled shared library)
...
pnpm build:fenster   # Compile fenster native bridge (native/fenster.dylib)
pnpm generate-font   # Regenerate bitmap font data from Cozette.bdf
```
...
- **Arrow functions**: Use `const foo = () => { ... }` (enforced by Biome, auto-fixable)
